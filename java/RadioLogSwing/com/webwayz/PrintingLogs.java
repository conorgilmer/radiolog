...
public class PrintingLogs extends JInternalFrame implements Printable {
	/**
	 * 
	 */
	...
	private String URL = "jdbc:mysql://localhost/opendata"; // localhost XAMMP MySql db opendata
	...
	public PrintingLogs(String query) {
		super("Printing Radio Logs", false, true, false, true);
		...
		cp.add(textArea);
		try {
			Class.forName("org.gjt.mm.mysql.Driver"); //mysqlconnect driver
		}
		...
		try {
			...
			textArea.append("=============== Radio Logs Information ===============\n\n");
			while (resultset.next()) {
				textArea.append("Name:     \t " + resultset.getString("name") + "\n" +
				        ...
				        "Time:     \t" + resultset.getString("times") + "\n\n");
			}
			textArea.append("=============== Radio Logs Information ===============");
			resultset.close();
			statement.close();
